...
Like `R`, `NumPy` and `pandas` can use vectorised processes that make working with large datasets more efficient.
...
```{r multiple-observations, echo = FALSE}
messy_data %>% knitr::kable('html')
```
...
- If you can control how the data is created, make sure you use the tidy data structure **from the beginning**.
- If you cannot control how the raw data is structured, wrangle it into the tidy format, ideally using a **scripted programming language**, like Python or R.
...
- Notice the two tabs: Two RAs conducted the surveys, one in 2013 and one in 2014. They track of the data in their own way in the tabs 2013 and 2014 respectively. Now you're the person in charge of this project and you want to be able to start analysing the data.
...
- **Comma delimited**: comma-separated value files (.csv) are plain text files where the columns are separated by commas
- **Tab delimited**: tab-separated value files (.tsv) are plain text files where the columns are separated by tabs (\t)
